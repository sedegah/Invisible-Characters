  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="pt-32 pb-16 px-6 max-w-7xl mx-auto">
        <div className="mb-12">
          <h1 className="text-4xl md:text-5xl font-bold text-foreground mb-4">Unicode Scanner</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-card border border-border rounded-xl p-6">
              <label className="block text-sm font-semibold text-foreground mb-3">Enter Text to Analyze</label>
              <textarea
                value={inputText}
                onChange={(e) => {
            ...
            {highlightedText && (
              <div className="bg-card border border-border rounded-xl p-6">
                <label className="block text-sm font-semibold text-foreground mb-3">Text with Suspicious Character Markers (⦿)</label>
                <div className="w-full min-h-24 max-h-40 bg-muted border border-border rounded-lg px-4 py-3 text-foreground font-mono text-sm overflow-y-auto whitespace-pre-wrap break-words">
                  {highlightedText}
                </div>
                <p className="text-xs text-foreground/60 mt-2">⦿ marks invisible or suspicious characters</p>
              </div>
            )}
            ...
            {detectedInvisible.length > 0 && (
              <div className="bg-destructive/10 border border-destructive/30 rounded-xl p-6">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <h2 className="text-lg font-semibold text-destructive">⚠️ Suspicious Characters Detected</h2>
                    <p className="text-sm text-destructive/80 mt-1">{detectedInvisible.length} invisible/suspicious character(s) found</p>
                  </div>
                </div>
            ...
            {cleanedText && detectedInvisible.length > 0 && (
              <div className="bg-green-500/10 border border-green-500/30 rounded-xl p-6">
                <label className="block text-sm font-semibold text-green-600 dark:text-green-400 mb-3">✓ Cleaned Text (Suspicious Characters Removed)</label>
            ...
            <div className="bg-card border border-border rounded-xl p-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-semibold text-foreground">Characters Found: {filtered.length}</h2>
                {characters.length > 0 && (
                  <button
                    onClick={() => {
                      setInputText("")
          ...
          {selectedChar && (
            <div className="lg:col-span-1">
              <div className="bg-card border border-border rounded-xl p-6 sticky top-32">
                <h3 className="text-lg font-semibold text-foreground mb-6">Character Details</h3>

                <div className="flex items-center justify-center mb-8 p-6 bg-muted rounded-lg border border-border">
                  <div className="text-6xl text-foreground">{selectedChar.char}</div>
                </div>

                <div className="space-y-4">
                  <div>
                    <label className="text-xs font-semibold text-foreground/60 uppercase">Unicode</label>
                    <div className="flex items-center justify-between mt-2">
                      <span className="text-lg text-foreground font-mono">{selectedChar.code}</span>
                      <button
                  ...
                  <div>
                    <label className="text-xs font-semibold text-foreground/60 uppercase">Hexadecimal</label>
                    <div className="flex items-center justify-between mt-2">
                      <span className="text-lg text-foreground font-mono">{selectedChar.hex}</span>
                      <button
                  ...
                  <div>
                    <label className="text-xs font-semibold text-foreground/60 uppercase">Name</label>
                    <p className="mt-2 text-foreground/80 text-sm">{selectedChar.name}</p>
                  </div>

                  <div>
                    <label className="text-xs font-semibold text-foreground/60 uppercase">Category</label>
                    <p className="mt-2 text-foreground/80 text-sm">{selectedChar.category}</p>
                  </div>

                  <div>
                    <label className="text-xs font-semibold text-foreground/60 uppercase">Block</label>
                    <p className="mt-2 text-foreground/80 text-sm">{selectedChar.block}</p>
                  </div>
                </div>
